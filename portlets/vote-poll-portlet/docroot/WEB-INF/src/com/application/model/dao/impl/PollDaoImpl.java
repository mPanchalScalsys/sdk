...
public class PollDaoImpl implements PollDao {
	...
	public List<Poll> getPollByPropertyId(long propertyId) {
		...
		return session.createSQLQuery("select *,CURDATE()>end_date AS flag from polls WHERE property_id="+ propertyId + " ORDER BY flag ASC, created_date DESC").addEntity(Poll.class).list();
	...
	public List<Poll> getPollByPropertyId(long propertyId, int start, int end) {
		...
		return session.createSQLQuery("select *,CURDATE()>end_date AS flag from polls WHERE property_id="+ propertyId + " ORDER BY flag ASC, created_date DESC").addEntity(Poll.class).setFirstResult(start).setMaxResults(end).list();
